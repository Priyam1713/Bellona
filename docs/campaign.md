# Campaign Log — Total War

> Standing orders for the conquest phase. Bellona is safe; now she must be
> *inevitable*. Doctrine unchanged (BELLONA.md Seven Laws) — new operating
> principle added:
>
> **Small battles, total victory.** Every engagement below is sized to ship
> alone — code + tests + a runnable demo + a receipt (benchmark/report).
...
|----------|-------|--------|
| I–IV     | Founding · Real Blood · Surfaces · Optimization | ✅ shipped (see CHANGELOG) |
| V        | Arsenal Expansion | ⚔️ active |
| VI       | Voices | queued |
| VII      | The War Room | queued |
| VIII     | Legions | queued |
| IX       | Foedus Rising (A2A) | queued |
| X        | Living Memory | queued |
| XI       | Self-Forging Legionaries | queued |
| XII      | The Armory Market | deferred-last-with-XIII |
| XIII     | Proof Against the Gods | **last** (needs live APIs/models) |

Sequencing rule: V → VI → VII run in parallel-friendly order; VIII depends on
...
## CAMPAIGN V — Arsenal Expansion
*Goal: an agent is only as dangerous as its toolbox. Fill it.*

### V1 — Tool ergonomics (force multiplier, do FIRST)
- **V1.1** `bellum::tools::tool!` declarative builder — define a tool from a
  plain async fn + doc comment (name/effect/schema inferred).
  *Victory:* existing 4 CLI tools rewritten with zero boilerplate loss.
- **V1.2** Conformance test-kit (`forge-testkit`): any tool must pass — spec
...
- **V2.1** Read-only set: `git_status`, `git_diff`, `git_log`, `git_blame`
  ...
  Policy: classified `file_read` → auto-allowed.
- **V2.2** Mutating set: `git_add`, `git_commit`, `git_branch`, `git_worktree`.
  Classified `file_write` → gated/yolo per existing law.
- **V2.3** E2E victory lap: scripted campaign edits a file → commits on a
  branch → assertions inspect a REAL temp repo (author, message, tree diff).
...
- **V3.1** `search_files` (regex/literal, glob-scoped, respects .gitignore,
  result cap + pagination) — walk-and-scan pure Rust first.
...
- **V3.3** Index hook: every search hit optionally appended to Archivum
  (`kind="episodic"`) — searches become memories.
...
### V4 — Web reading (HTTP-level browsing)
- **V4.1** `web_fetch`: GET-only, 2 MB cap, 15 s timeout, HTML→text
  (`html2text`), returns title + readable body.
- **V4.2** SSRF shield (critical small thing): resolve host → refuse
...
- **V4.4** `web_search` adapter (Brave/DuckDuckGo HTML) behind explicit
  enable-flag — search engines are opt-in, never ambient.
- **V4.5 (stretch)** CDP-driven `browser_navigate/act` via `chromiumoxide`
  for JS-heavy pages — enters War-Room territory (VII.6 shares this driver).
...
## CAMPAIGN VI — Voices (Discord/Slack)
...
- **VI.0** Refactor: extract `ChannelTransport` trait from TelegramTransport
  ...
  not a special case.
- **VI.1** Discord adapter: minimal Gateway v10 client over tokio-tungstenite —
  IDENTIFY, HEARTBEAT (respect server heartbeat_interval!), resume on close,
  MESSAGE_CREATE → Inbound. No mega-framework dependency.
- **VI.2** Slack adapter: Socket Mode (wss) with `apps.connections.open`,
  event ACKs within 3 s (miss = duplicate delivery — dedupe on event_id),
  `chat.postMessage` outbound.
- **VI.3** Router config: `channels.yaml` mapping channel→agent profile +
...
- **VI.5** Tests: frame codec unit tests + local WS echo-server integration;
  reconnect storm simulation (kill socket 5×, transport must self-heal).
...
## CAMPAIGN VII — The War Room (watch your agent work)
*Goal: eyes on the battle — and take the wheel when it stalls.*
...
- **VII.2** Live stream: SSE endpoint piping EventBus → foedus AG-UI events
...
- **VII.4** Static war-room UI (single index.html + vanilla JS/htmx, served by
  the server — zero node build step): channel view, activity feed, approve
...
takes the wheel once, verifies the chain — all in a browser.

---

## CAMPAIGN VIII — Legions (agent teams)
*Goal: one Centurion, many specialists, zero chaos.*

- **VIII.1** `CenturioStrategy` in bellum: decomposes goal → worker briefs
...
- **VIII.3** Federated budgets: parent allocates child Aerariums; child
  overrun trips child breaker AND surfaces to parent — never silently eats
  the fleet budget.
- **VIII.4** Fleet governor: max concurrent workers, per-worker no-progress
  ...
  ticket cancellation — extend to spawned loops).
...
- *Watch for:* recursive spawning (workers spawning workers — forbid unless
...
## CAMPAIGN IX — Foedus Rising (A2A fluency)
...
- **IX.1** Delegatee HTTP server (axum) implementing foedus A2A shapes;
  **idempotency ledger**: duplicate `idempotency_key` returns cached verdict
  (Annales-backed lookup) — retries must never double-fire effects.
...
- **IX.4** Capability negotiation: task routed only when card.skills ⊇ task
  requirements; stale-card TTL + refresh.
- **IX.5** E2E: two `bellona serve` processes — one delegates "research X"
...
- *Watch for:* clock skew in TTLs, partial-failure semantics (delegatee died
  post-audit pre-reply → idempotent replay), secret bleed in task contexts
  (context sanitizer).

---

## CAMPAIGN X — Living Memory
...
- **X.3** Hybrid recall: FTS ∪ vector → reciprocal-rank fusion + recency
...
## CAMPAIGN XI — Self-Forging Legionaries
*Goal: the agent that builds its own weapons — under inspection.*

- **XI.1** Trigger wiring: repeated `tool_not_found` observations (N≥2) prompt
  the strategy to draft an Officina proposal (name/script/battery plan).
- **XI.2** Battery generation: model proposes cases; Ludus REQUIRES ≥3 cases
  including adversarial set (empty args, escape-attempt path, oversized input)
  — a tool untested against malice is untested.
...
- *Watch for:* battery overfitting (cases mirroring implementation instead of
  intent — require intent descriptions per case), tool-name squatting,
  forged-tool calling forged-tools (depth limit 1 until XII).

---

## CAMPAIGN XII — The Armory Market (WASM) *(paired with XIII at the end)*
*Goal: third-party power without third-party trust.*

- **XII.1** WIT contract v1: exports `describe()/call(input)->output`;
  imports granted individually — `cap_log`, `cap_kv`, `cap_http(host
...
## CAMPAIGN XIII — Proof Against the Gods *(strictly last)*
*Goal: receipts against real intelligence. Build everything first; measure it second.*

- **XIII.1** Prereq: GitHub token with **workflow scope** (current blocker —
  flip Repository permissions → Workflows: Read & write).
- **XIII.2** Live runner: `bellona colosseum --provider openai|anthropic|
  ollama [--model ...]` — Aerarium cost-caps enforced per suite; API keys env-
  only; redacted artifacts.
- **XIII.3** Nightly matrix in CI (openai/anthropic/ollama × seed + swe-mini
  suites); failures open issues with full replay bundles.
- **XIII.4** Suites: `seed-live` (plumbing vs real models), `swe-mini`
  (temp-repo bugfixes verified by real test runs — SWE-bench-shaped, private
...
- **XIII.6** Public receipts: auto-generated `docs/receipts.md` — harness-
...
| Context window gluttony | III–X | chunked readers, summary-only parents |
...
reads tomorrow's public receipts showing pass^k — without ever trusting a
vendor promise.
